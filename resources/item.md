...
in OneDrive are returned as **driveItem** resources. They can be accessed by their **id**
using the `/items/{item-id}` syntax, or by their file system path using the
`/drive/root:/path/to/file` syntax.

DriveItems have [facets](../facets/facets.md) that provide data about the item's
...
DriveItems with the **folder** facet act as containers of items and therefore
...
| [Copy an Item](../items/copy.md)                    | `POST /drive/items/{id}/copy`                          | `POST /drive/root:/{path}:/copy`                 |
| [Download an Item's contents](../items/download.md) | `GET /drive/items/{id}/content`                        | `GET /drive/root:/{path}:/content`               |
| [Search for an Item](../items/search.md)            | `GET /drive/items/{id}/search`                         | `GET /drive/root:/{path}:/search`                |
| [View changes on an Item][item-changes]             | `GET /drive/items/{id}/delta`                          | `GET /drive/root:/{path}:/delta`                 |
| [Get thumbnails for an Item][get-thumbnails]        | `GET /drive/items/{id}/thumbnails`                     | `GET /drive/root:/{path}:/thumbnails`            |
| [Sharing by link](../items/sharing_createLink.md)   | `POST /drive/items/{id}/createLink`                    | `POST /drive/root:/{path}:/createLink`           |
...
**Note:** The `@microsoft.graph.downloadUrl` is a short-lived URL and can't be cached. 
...
| **children**      | [driveItem](item.md) collection            | Collection containing Item objects for the immediate children of Item. Only items representing folders have children.                                    |
